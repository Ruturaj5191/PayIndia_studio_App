...
export default function UpdateElectricityDetailsScreen() {
    ...
    const [formData, setFormData] = useState({
        ...
        nameCorrection: {
            currentName: "Sanjay Kumar Rao", // Mocked initial data
            correctedName: "",
            reason: "",
        },
        loadChange: {
            requestType: "", // Increase Load, Decrease Load, Category Correction
        },
        declaration: false,
    });
    ...
    const handleFetchConsumerData = () => {
        if (!formData.consumerNumber || !formData.mobileNumber || !documents.previousBill) {
            Alert.alert("Details Required", "Please fill all fields and upload previous bill");
            return;
        }
        if (!isOtpVerified) {
    ...
    const handleSaveAndContinue = () => {
        ...
        if (!formData.email && !formData.address.city) {
            Alert.alert("Action Required", "Please update at least one detail or confirm existing info");
            // In a real app we'd check if any field was touched or changed
        }
        ...
        if ((formData.address.street || formData.address.area) && !documents.addressProof) {
            // Technically if they touch address they should upload proof
        }
    ...
    const handleSubmitRequest = () => {
        ...
        setTimeout(() => {
            const ref = "ELE" + Math.floor(Math.random() * 9000000 + 1000000);
            setReferenceId(ref);
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={[styles.successContainer, { flex: 1, justifyContent: 'center', alignItems: 'center' }]}>
                    ...
                    <Text style={styles.successSubtitle}>Your electricity details update request has been received</Text>
                    ...
                    <View style={styles.infoBox}>
                        ...
                        <Text style={styles.infoBoxText}>Estimated processing time: 3-5 working days</Text>
                    </View>
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerCenter}>
                        ...
                        <Text style={styles.headerSubtitle}>Electricity Bill Update</Text>
                    </View>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                    ...
                    {currentStep === 1 && (
                        <View style={styles.docsRequiredBox}>
                            ...
                            <View style={styles.docsList}>
                                <Text style={styles.docBullet}>• Consumer Number / Customer ID (ग्राहक क्रमांक)</Text>
                                <Text style={styles.docBullet}>• Previous Electricity Bill Copy (मागील वीज बिलाची प्रत)</Text>
                                <Text style={styles.docBullet}>• Registered Mobile Number (नोंदणीकृत मोबाईल नंबर)</Text>
                            </View>
                        </View>
                    )}
                    ...
                    {currentStep === 1 && (
                        <View>
                            <SectionTitle title="Consumer Verification" icon="shield-checkmark" />
                            <View style={styles.formCard}>
                                <Label text="Consumer Number / Customer ID *" />
                                <Input
                                ...
                                <View style={{ marginTop: 20 }}>
                                    <Label text="Upload Previous Electricity Bill *" />
                                    <DocUploadItem
                            ...
                            <TouchableOpacity style={styles.actionBtn} onPress={handleFetchConsumerData}>
                                <LinearGradient colors={['#0D47A1', '#1565C0']} style={styles.btnGrad}>
                    ...
                    {currentStep === 2 && (
                        <View>
                            ...
                            <View style={styles.formCard}>
                                <View style={{ flexDirection: 'row', gap: 10 }}>
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <Input value={formData.address.flatNo} onChangeText={(v: string) => setFormData({ ...formData, address: { ...formData.address, flatNo: v } })} placeholder="No." />
                                    </View>
                                    <View style={{ flex: 2 }}>
                                        <Label text="Street" />
                                ...
                                <View style={{ marginTop: 10 }}>
                                    ...
                                    <DocUploadItem
                                        title="Address Proof"
                                        hint="Registry / Tax Receipt / Ration Card"
                            ...
                            <SectionTitle title="4. Load / Category Change" icon="flash" />
                            <View style={styles.formCard}>
                                ...
                                >
                                    <Text style={[styles.dropdownText, !formData.loadChange.requestType && { color: '#94A3B8' }]}>
                                        {formData.loadChange.requestType || "Select Change Type"}
                                    </Text>
                                    <Ionicons name="chevron-down" size={20} color="#666" />
                                </TouchableOpacity>
                                <Label text="Supporting Document" />
                                <DocUploadItem
                                    title="Load Support Doc"
                                    hint="Electrician Report / Declaration"
                                    isUploaded={!!documents.loadSupport}
                                    filename={documents.loadSupport?.name}
                                    onUpload={() => pickDocument('loadSupport')}
                                    onRemove={() => removeDocument('loadSupport')}
                                />
                            </View>
                    ...
                    {currentStep === 3 && (
                        <View>
                            ...
                            <View style={styles.summaryCard}>
                                ...
                                <SummarySection title="Email" value={formData.email || "Not Provided"} onEdit={() => setCurrentStep(2)} />
                                <SummarySection
                                    title="Billing Address"
                                    value={`${formData.address.flatNo}, ${formData.address.street}, ${formData.address.city} - ${formData.address.pincode}`}
                                    onEdit={() => setCurrentStep(2)}
                                />
                                {formData.nameCorrection.correctedName && (
                                    <SummarySection title="Corrected Name" value={formData.nameCorrection.correctedName} onEdit={() => setCurrentStep(2)} />
                                )}
                                <SummarySection title="Request Type" value={formData.loadChange.requestType || "Profile Update Only"} onEdit={() => setCurrentStep(2)} />

                                <View style={styles.divider} />
                                <Text style={styles.attachedDocsTitle}>Attached Documents:</Text>
                                <View style={styles.attachedDocsList}>
                                    {documents.previousBill && <Text style={styles.attachedDoc}>• Electricity Bill (मागील वीज बिल)</Text>}
                                    {documents.identityProof && <Text style={styles.attachedDoc}>• Identity Proof (Name Correction)</Text>}
                                    {documents.addressProof && <Text style={styles.attachedDoc}>• Address Proof</Text>}
                                    {documents.loadSupport && <Text style={styles.attachedDoc}>• Load Request Document</Text>}
                                </View>
                            </View>
                            ...
                            >
                                ...
                                <Text style={styles.declarationText}>I confirm that the above information is correct and I understand incorrect details may lead to rejection.</Text>
                            </TouchableOpacity>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            setFormData({ ...formData, loadChange: { ...formData.loadChange, requestType: item } });
                                            setShowRequestModal(false);
                                        }}
                                    >
                                        <Text style={[styles.optionText, formData.loadChange.requestType === item && styles.optionTextActive]}>
                                            {item}
                                        </Text>
                                        {formData.loadChange.requestType === item && (
...
const styles = StyleSheet.create({
    ...
    attachedDocsList: { gap: 5 },
    attachedDoc: { fontSize: 12, color: '#64748B' },
